...
- FreeFrame review now opens from a compact work-item widget into a focused, near-full-screen overlay; linked, unlinked, and permission-limited states use Plane locale keys.
...
- Toast localization no longer probes nonexistent generated keys for already-localized text, preventing false missing-key warnings such as `toast_messages.id`.
...
- Plane Messenger Gateway now rejects placeholder secrets, installs its declared dependencies, binds the optional overlay to loopback, and routes startup through a hardened entrypoint.
