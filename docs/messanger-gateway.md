...
`services/plane-messanger-gateway` is an optional sidecar service for the
Plane stack. It currently enables the VK adapter only; it does not modify Plane
frontend, backend, web, API, or database schema.

## Configuration

Create `services/plane-messanger-gateway/.env` from its `.env.example` and set
...
`plane-messanger-gateway-data`. Never copy that data between instances.
...
Set `PLANE_MESSANGER_GATEWAY_PORT` only when another host-loopback port is
