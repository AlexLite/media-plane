...
export const IssueBlockLabels = observer(function IssueBlockLabels({ labelIds, shouldShowLabel = false }: Props) {
  ...
  const labelsString = labels.length > 0 ? labels.map((label) => label.name).join(", ") : t("space_issue.no_labels");

  if (labels.length <= 0)
    return (
      <Tooltip position="top" tooltipHeading={t("space_issue.labels")} tooltipContent={t("common.none")}>
        <div
        ...
        >
          ...
          {shouldShowLabel && <span>{t("space_issue.no_labels")}</span>}
        </div>
      </Tooltip>
    );
  ...
  return (
    <div className="flex h-5 w-full flex-wrap items-center gap-2 overflow-hidden">
      {labels.length <= 2 ? (
        ...
        >
          <Tooltip position="top" tooltipHeading={t("space_issue.labels")} tooltipContent={labelsString}>
            <div className="flex h-full items-center gap-1.5 text-secondary">
              ...
              {t("space_issue.labels_count", { count: labels.length })}
            </div>
          </Tooltip>
        </div>
      )}
