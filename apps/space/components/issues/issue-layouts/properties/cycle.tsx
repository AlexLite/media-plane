...
export const IssueBlockCycle = observer(function IssueBlockCycle({ cycleId, shouldShowBorder = true }: Props) {
  ...
  return (
    <Tooltip tooltipHeading={t("space_issue.cycle")} tooltipContent={cycle?.name ?? t("space_issue.no_cycle")}>
      <div
        className={cn(
      ...
      >
        <div className="flex w-full items-center gap-1.5 text-11">
          ...
          <div className="max-w-40 truncate">{cycle?.name ?? t("space_issue.no_cycle")}</div>
        </div>
      </div>
    </Tooltip>
  );
