    ...
    <ModalCore isOpen={isOpen} handleClose={handleClose} position={EModalPosition.CENTER} width={EModalWidth.XXL}>
      <form onSubmit={handleSubmit(onSubmit)} className="p-5">
        <div className="flex items-center justify-between">
          <h3 className="text-h5-medium leading-6 text-primary">{t("notification.snooze.modal.title")}</h3>
        ...
        <div className="mt-5 flex flex-col gap-3 md:!flex-row md:items-center">
          <div className="flex-1 pb-3 md:pb-0">
            ...
            <Controller
              name="date"
              control={control}
              rules={{ required: t("notification.snooze.modal.select_date_required") }}
              render={({ field: { value, onChange } }) => (
                <DateDropdown
                  value={value || null}
                  placeholder={t("notification.snooze.modal.select_date")}
                  onChange={(val) => {
          ...
          <div className="flex-1">
            ...
            <Controller
              control={control}
              name="time"
              rules={{ required: t("notification.snooze.modal.select_time_required") }}
              render={({ field: { value, onChange } }) => (
                <CustomSelect
                  value={value}
                  onChange={onChange}
                ...
                >
                  ...
                  {getTimeStamp().length > 0 ? (
                    ...
                    <p className="p-3 text-center text-secondary">{t("notification.snooze.modal.no_available_time")}</p>
                  )}
                </CustomSelect>
              )}
            />
