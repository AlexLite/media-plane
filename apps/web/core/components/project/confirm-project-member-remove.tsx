        ...
        <div className="sm:flex sm:items-start">
          ...
          <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
            <h3 className="text-16 leading-6 font-medium text-primary">
              {isCurrentUser
                ? t("project_settings.members.leave_title")
                : t("project_settings.members.remove_title", { name: data?.display_name })}
            </h3>
            <div className="mt-2">
              <p className="text-13 text-secondary">
                {isCurrentUser
                  ? t("project_settings.members.leave_confirmation", { name: currentProjectDetails?.name })
                  : t("project_settings.members.remove_confirmation", { name: data?.display_name })}
              </p>
            </div>
          </div>
        </div>
