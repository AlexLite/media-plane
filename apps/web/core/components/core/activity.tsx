...
const getInboxUserActivityMessage = (activity: IIssueActivity, showIssue: boolean, t: (key: string) => string) => {
  const inboxActivityMessage = {
    declined: {
      ...
      noIssue: t("activity_feed.inbox.declined_no_issue"),
    },
    snoozed: {
      showIssue: t("activity_feed.inbox.snoozed_issue"),
      noIssue: t("activity_feed.inbox.snoozed_no_issue"),
    },
    accepted: {
      showIssue: t("activity_feed.inbox.accepted_issue"),
      noIssue: t("activity_feed.inbox.accepted_no_issue"),
    },
    markedDuplicate: {
      showIssue: t("activity_feed.inbox.declined_issue"),
      noIssue: t("activity_feed.inbox.declined_duplicate_no_issue"),
    },
  };
...
const getActivityDetails = (
  ...
  assignees: {
    message: (activity, showIssue) => {
      if (activity.old_value === "")
        return (
          <>
            {t("activity_feed.added_new_assignee")} <UserLink activity={activity} />
            {showIssue && (
              <>
                {" "}
                {t("activity_feed.to")} <IssueLink activity={activity} />
              </>
            )}
          </>
        );
        ...
        return (
          <>
            {t("activity_feed.removed_assignee")} <UserLink activity={activity} />
            {showIssue && (
              <>
                {" "}
                {t("activity_feed.from")} <IssueLink activity={activity} />
              </>
            )}
          </>
        );
  ...
  archived_at: {
    message: (activity) => {
      if (activity.new_value === "restore")
        return (
          <>
            {t("activity_feed.restored")} <IssueLink activity={activity} />
          </>
        );
      else
        return (
          <>
            {t("activity_feed.archived")} <IssueLink activity={activity} />
          </>
        );
    },
  ...
  attachment: {
    message: (activity, showIssue) => {
      if (activity.verb === "created")
        return (
          <>
            {t("activity_feed.uploaded_attachment")}
            {showIssue && (
              <>
                {" "}
                {t("activity_feed.to")} <IssueLink activity={activity} />
        ...
        return (
          <>
            {t("activity_feed.removed_attachment")}
            {showIssue && (
              <>
                {" "}
                {t("activity_feed.from")} <IssueLink activity={activity} />
  ...
  description: {
    message: (activity, showIssue) => (
      <>
        {t("activity_feed.updated_description")}
        {showIssue && (
          <>
            {" "}
            {t("activity_feed.of")} <IssueLink activity={activity} />
          </>
        )}
      </>
    ),
  ...
  estimate_point: {
    message: (activity, showIssue) => {
      if (!activity.new_value)
        return (
          <>
            {t("activity_feed.removed_estimate")}
            {showIssue && (
              <>
                {" "}
                {t("activity_feed.from")} <IssueLink activity={activity} />
        ...
        return (
          <>
            {t("activity_feed.set_estimate_to")} {activity.new_value}
            {showIssue && (
              <>
                {" "}
                {t("activity_feed.for")} <IssueLink activity={activity} />
              </>
            )}
          </>
        );
  ...
  issue: {
    message: (activity) => {
      if (activity.verb === "created")
        return (
          <>
            {t("activity_feed.created")} <IssueLink activity={activity} />
          </>
        );
      else if (activity.verb === "converted")
        return (
          <>
            {t("activity_feed.converted")} <IssueLink activity={activity} /> {t("activity_feed.to_epic")}
          </>
        );
      else
        return (
          <>
            {t("activity_feed.deleted")} <IssueLink activity={activity} />
          </>
        );
    },
  ...
  epic: {
    message: (activity) => {
      if (activity.verb === "created")
        ...
        return (
          <>
            {t("activity_feed.converted")} <IssueLink activity={activity} /> {t("activity_feed.to_work_item")}
          </>
        );
      else
        return (
      ...
      if (activity.old_value === "")
        return (
          <span className="overflow-hidden">
            {t("activity_feed.added_new_label")}{" "}
  ...
  cycles: {
    message: (activity, showIssue, workspaceSlug) => {
      if (activity.verb === "created")
        ...
        return (
          <>
            <span className="flex-shrink-0 whitespace-nowrap">{t("activity_feed.set_cycle_to")} </span>
            <a
              href={`/${workspaceSlug}/projects/${activity.project}/cycles/${activity.new_identifier}`}
              target="_blank"
              rel="noopener noreferrer"
        ...
        return (
          <>
            {t("activity_feed.removed")} <IssueLink activity={activity} /> {t("activity_feed.from_cycle")}{" "}
            <a
  ...
  modules: {
    message: (activity, showIssue, workspaceSlug) => {
      if (activity.verb === "created")
        ...
        return (
          <>
            {t("activity_feed.removed")} <IssueLink activity={activity} /> {t("activity_feed.from_module")}{" "}
            <a
  ...
  name: {
    message: (activity, showIssue) => (
      <>
        {t("activity_feed.set_title_to")} <span className="break-all">{activity.new_value}</span>
        {showIssue && (
          <>
            {" "}
            {t("activity_feed.of")} <IssueLink activity={activity} />
  ...
  priority: {
    message: (activity, showIssue) => (
      <>
        {t("activity_feed.set_priority_to")}{" "}
  ...
  relates_to: {
    message: (activity, showIssue) => {
      if (activity.old_value === "")
        ...
        return (
          <>
            {t("activity_feed.removed_relation_from")}{" "}
            <span className="font-medium whitespace-nowrap text-primary">{activity.old_value}</span>.
          </>
        );
    },
  ...
  blocking: {
    message: (activity, showIssue) => {
      if (activity.old_value === "")
        ...
        return (
          <>
            {t("activity_feed.removed_blocking_work_item")}{" "}
            <span className="font-medium whitespace-nowrap text-primary">{activity.old_value}</span>.
          </>
        );
    },
  ...
  start_date: {
    message: (activity, showIssue) => {
      if (!activity.new_value)
        return (
          <>
            {t("activity_feed.removed_start_date")}
            {showIssue && (
              <>
                {" "}
                {t("activity_feed.from")} <IssueLink activity={activity} />
        ...
        return (
          <>
            {t("activity_feed.set_start_date_to")}{" "}
  ...
  target_date: {
    message: (activity, showIssue) => {
      if (!activity.new_value)
        return (
          <>
            {t("activity_feed.removed_due_date")}
            {showIssue && (
              <>
                {" "}
                {t("activity_feed.from")} <IssueLink activity={activity} />
        ...
        return (
          <>
            {t("activity_feed.set_due_date_to")}{" "}
            <span className="font-medium whitespace-nowrap text-primary">
              {renderFormattedDate(activity.new_value)}
            </span>
            {showIssue && (
  ...
  inbox: {
    message: (activity, showIssue) => (
      <>
        ...
        {activity.verb === "2" && ` ${t("activity_feed.inbox.from_intake_by_marking_duplicate")}`}
      </>
    ),
