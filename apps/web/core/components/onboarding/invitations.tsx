  ...
  return invitations && invitations.length > 0 ? (
    <div className="space-y-4">
      <div className="mx-auto space-y-1 py-4 text-center">
        <h3 className="text-24 font-bold text-primary">{t("onboarding_invites.title")}</h3>
        <p className="font-medium text-placeholder">{t("onboarding_invites.description")}</p>
      </div>
      <div>
        {invitations &&
      ...
      >
        {isJoiningWorkspaces ? <Spinner height="20px" width="20px" /> : t("onboarding_invites.continue_to_workspace")}
      </Button>
      <div className="mx-auto mt-4 flex items-center sm:w-96">
        <hr className="w-full border-strong" />
        <p className="mx-3 flex-shrink-0 text-center text-13 text-placeholder">{t("onboarding_invites.or")}</p>
        <hr className="w-full border-strong" />
      </div>
      <Button
        variant="ghost"
      ...
      >
        {t("onboarding_invites.create_workspace")}
      </Button>
    </div>
  ) : (
    <div>{t("onboarding_invites.no_invitations")}</div>
  );
}
