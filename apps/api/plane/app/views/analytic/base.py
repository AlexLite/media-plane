...
class AnalyticsEndpoint(BaseAPIView):
    ...
    def get(self, request, slug):
        ...
        if not x_axis or not y_axis or x_axis not in VALID_ANALYTICS_FIELDS or y_axis not in VALID_YAXIS:
            return Response(
                {"error": "Укажите корректные измерения для осей X и Y"},
        ...
        if segment and (segment not in VALID_ANALYTICS_FIELDS or x_axis == segment):
            return Response(
                {"error": "Сегмент и ось X не могут совпадать, а сегмент должен быть корректным"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class ExportAnalyticsEndpoint(BaseAPIView):
    ...
    def post(self, request, slug):
        ...
        return Response(
            {"message": f"Когда экспорт будет готов, мы отправим его на {str(request.user.email)}"},
            status=status.HTTP_200_OK,
        )
